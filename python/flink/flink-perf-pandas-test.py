...
def main():
    ...
    t_env.get_config().get_configuration().set_integer("python.fn-execution.arrow.batch.size",
                                                       10000)
    ...
    t_env.register_function("pandas_inc", pandas_inc)

    num_rows = 1000000000
    t_env.from_table_source(RangeTableSource(1, num_rows, 1)).alias("id") \
        .select("pandas_inc(id)") \
        .insert_into("sink")

    beg_time = time.time()
    t_env.execute("Pandas UDF")
    print("PyFlink Pandas UDF pandas_inc() consume time: " + str(time.time() - beg_time))
